...
import { CallArgs, Doc, Nawah, Res, SDKConfig } from '@nawah/javascript';
import 'nativescript-websockets';
import { Observable } from 'rxjs';
import { webSocket } from 'rxjs/webSocket';

Nawah.setCallables({
  websocketInit: (nawah: Nawah, config: SDKConfig) => webSocket(config.api),
          ...
          const fileUpload: Observable<Res<Doc>> = new Observable<Res<Doc>>(
            (observer) => {
              const params = [
                ...
                { name: 'type', value: (files[attr][Number(i)] as any).type },
                {
                  name: 'lastModified',
