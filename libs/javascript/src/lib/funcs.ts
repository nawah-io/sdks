...
import { CallArgs, Doc, Require, Res, SDKConfig } from './nawah.models';
...
export function populateFilesUploads(
  nawah: NawahBase,
  config: SDKConfig,
  callArgs: Require<CallArgs, 'doc' | 'sid' | 'token'>
  ...
  for (const attr of Object.keys(callArgs.doc)) {
    if (callArgs.doc[attr] instanceof FileList) {
      nawah._log(nawah, config, 'log', 'Detected FileList for doc attr:', attr);
      files[attr] = callArgs.doc[attr] as FileList;
      callArgs.doc[attr] = [];
      nawah._log(
        nawah,
        config,
      ...
      for (let i = 0; i < files[attr].length; i++) {
        (callArgs.doc[attr] as Array<unknown>).push(files[attr][i].name);
        nawah._log(
          nawah,
          config,
          'log',
        ...
        const fileUpload: Observable<Res<Doc>> = new Observable((observer) => {
          ...
          xhr.onload = () => {
            ...
            (callArgs.doc[attr] as Array<unknown>)[i] = {
          ...
          xhr.open(
            'POST',
            `${config.api.replace('ws', 'http').replace('/ws', '')}/file/create`
          );
  ...
  nawah._log(nawah, config, 'log', 'Populated filesObservables:', filesUploads);
